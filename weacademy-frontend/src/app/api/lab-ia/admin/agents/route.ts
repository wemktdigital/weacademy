...
export async function GET(request: NextRequest) {
  try {
    ...
    // Buscar agentes usando service role para bypass RLS
    const { data: agents, error, count } = await serviceRoleSupabase
      .from('lab_agents')
      .select('*', { count: 'exact' })
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Preparar dados para inserção (knowledge_base_files já pode ser salvo na tabela)
    const agentData = {
      ...validatedData,
    ...
    // Criar agente usando service role para bypass RLS
